...
FONNTE_COMMUNITY_ID = os.environ.get('FONNTE_COMMUNITY_ID', '')
FONNTE_COMMUNITY_LINK = os.environ.get('FONNTE_COMMUNITY_LINK', 'https://chat.whatsapp.com/FJ8pX835Hwn32Y4eRXG91N')
...
async def register(payload: UserRegister):
    ...
    # referral_code = first 8 chars of UUID, always unique enough, used as lookup key
    doc["referral_code"] = user_id[:8].lower()
    doc["role"] = "user"
    await db.users.insert_one(doc)
    token = create_jwt_token(user_id, payload.email.lower())
    return {
        "token": token,
        "user": {
            "id": user_id,
...
async def _send_whatsapp(phone: str, message: str) -> bool:
    """Send WhatsApp message via Fonnte API (https://fonnte.com)."""
    if not FONNTE_TOKEN or not _HTTPX_AVAILABLE:
        return False
    try:
        async with httpx.AsyncClient(timeout=10) as client:
            r = await client.post(
        ...
        logger.warning(f"WhatsApp (Fonnte) send failed: {e}")
        return False
        ...
        try:
            ...
            for post in due:
                ...
                if notif and notif.get("notifications_enabled", True):
                    ...
                    if notif.get("whatsapp_phone"):
                        # WhatsApp doesn't support HTML
                        plain = msg.replace("<b>", "*").replace("</b>", "*").replace("<i>", "").replace("</i>", "").replace("<br>", "\n")
                        await _send_whatsapp(notif["whatsapp_phone"], plain)
...
@api_router.post("/webhooks/fonnte")
async def fonnte_webhook(request: Request):
    """Fonnte calls this when a message arrives in our WhatsApp Community."""
    try:
        body = await request.json()
    except Exception:
        return {"status": "ok"}
    ...
    if not isgroup or not member:
        return {"status": "ignored"}

    # Only process messages from our community group
    if FONNTE_COMMUNITY_ID and sender != FONNTE_COMMUNITY_ID:
        return {"status": "ignored"}

    # Normalize phone: strip non-digits, convert leading 0 → 62
    phone = _re.sub(r'\D', '', member)
    ...
    # Match by last 9 digits to handle 62/08 prefix differences
    suffix = phone[-9:]
    notif = await db.notification_settings.find_one(
        {"whatsapp_phone": {"$regex": suffix + r"$"}}
    )
    if notif:
        user_id = notif["user_id"]
        await db.users.update_one(
            {"id": user_id},
            {"$set": {"community_verified": True, "community_verified_at": now_iso()}}
        )
        logger.info(f"Community verified: user {user_id} via WA {phone}")
...
async def community_status(current_user: dict = Depends(get_current_user)):
    ...
    return {
        ...
        "community_link": FONNTE_COMMUNITY_LINK,
        "discount_pct": 5,
    }
