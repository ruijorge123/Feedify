...
const STYLE_PRESETS = [
  "Minimal Clean", "Minimal Korean", "Editorial Bold", "Vibrant Pop", "Neon Street",
  "Lifestyle Natural", "Lifestyle Social", "Luxury Editorial", "Luxury Spa", "Luxury Korean",
  "Dark Moody", "Warm Artisan",
];
const CONCEPT_OPTIONS = [
  { id: "hero_studio",     name: "Hero Studio",       desc: "Produk jadi bintang utama, studio bersih premium", icon: "⭐" },
  { id: "flat_lay",        name: "Flat Lay",           desc: "Bird's eye overhead, props tersusun artistik", icon: "🔲" },
  { id: "lifestyle_scene", name: "Lifestyle Scene",   desc: "Produk dalam konteks kehidupan nyata audience", icon: "🌿" },
  { id: "shadow_drama",    name: "Shadow Drama",      desc: "Cahaya dramatis, bayangan bold cinematik", icon: "🎭" },
  { id: "abstract_brand",  name: "Abstract Brand",    desc: "Bentuk geometris bold, brand identity statement", icon: "◈" },
  { id: "texture_surface", name: "Texture & Surface", desc: "Produk di atas permukaan premium bertekstur", icon: "◻" },
  { id: "nature_botanical",name: "Nature & Botanical",desc: "Dikelilingi elemen alam dan tanaman hidup", icon: "🌱" },
  { id: "urban_context",   name: "Urban Context",     desc: "Latar belakang kota dan street aesthetic", icon: "🏙" },
  { id: "cutout_pop",      name: "Cut-Out Pop",       desc: "Produk melayang di background warna bold", icon: "💥" },
  { id: "duotone_mood",    name: "Duotone Mood",      desc: "Filter dua warna brand membungkus seluruh foto", icon: "◑" },
  { id: "minimal_type",    name: "Minimal & Type",    desc: "Tipografi bold mendominasi, produk sebagai aksen", icon: "Aa" },
  { id: "behind_glass",    name: "Behind Glass",      desc: "Efek kaca atau cermin yang dramatis dan premium", icon: "🪟" },
];
...
export default function BannerGeneratorPage() {
  ...
  const previewPrompt = async () => {
    ...
    try {
      ...
      console.error("preview-banner error:", err?.response?.data || err?.message || err);
  ...
  return (
    <div className="space-y-6" data-testid="banner-generator-page">
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6 animate-fade-up min-w-0">
          <div className="feedify-card p-6 space-y-5">
            {/* Express fields — always visible */}
            <div className="flex items-center justify-between">
              <h3 className="font-heading text-lg font-semibold text-brand">Info Konten</h3>
              <span className="text-[10px] uppercase tracking-widest text-brand-gold font-bold bg-brand-gold/10 px-2.5 py-1 rounded-full">Express</span>
            </div>

            <Field label="Nama Produk *">
              <input required data-testid="banner-product-name" value={form.product_name}
            ...
            {/* Auto-filled badges from brand profile */}
            <div className="flex flex-wrap gap-2 pt-1">
              <div className="flex items-center gap-1.5 text-xs bg-brand-sand px-3 py-1.5 rounded-full text-brand-light">
                ...
                Gaya: <span className="font-semibold text-brand">{form.style_preset || "Minimal Clean"}</span>
                <span className="text-stone-300">·</span>dari Brand Profile
              </div>
              {form.call_to_action && (
                <div className="flex items-center gap-1.5 text-xs bg-brand-sand px-3 py-1.5 rounded-full text-brand-light">
                  <CheckCircle size={12} weight="fill" className="text-green-500" />
                  CTA: <span className="font-semibold text-brand truncate max-w-[120px]">{form.call_to_action}</span>
                  <span className="text-stone-300">·</span>dari Brand Profile
                </div>
              )}
            </div>
            ...
            <button
              ...
              className="w-full flex items-center justify-between px-4 py-3 rounded-xl border border-dashed border-brand-sand hover:border-brand-light hover:bg-brand-sand/30 transition-all text-sm text-stone-500 hover:text-brand"
            >
              <div className="flex items-center gap-2">
                <SlidersHorizontal size={15} weight="duotone" />
                <span className="font-medium">Sesuaikan lebih lanjut</span>
                {(form.subheadline || form.description || form.features.length > 0) && (
            ...
            {/* Advanced fields */}
            {showAdvanced && (
              <div className="space-y-5 pt-1 border-t border-brand-sand/60">
          ...
          <div className="feedify-card p-6 space-y-4" ref={photoSectionRef}>
            <div className="flex items-center gap-2">
              <h3 className="font-heading text-lg font-semibold text-brand">Foto Produk</h3>
            ...
            <label htmlFor="banner-photo" data-testid="banner-photo-label" className={`block cursor-pointer border-2 border-dashed rounded-xl p-6 hover:border-brand-light text-center transition-colors ${photo ? "border-brand-light" : "border-red-300 bg-red-50/30"}`}>
              {photo ? <div><img src={photo} alt="produk" className="max-h-48 mx-auto rounded-lg" /><div className="text-xs text-stone-500 mt-2">Klik untuk ganti</div></div> : (
                <div className="py-4"><Camera size={32} className="mx-auto text-red-400 mb-2" weight="duotone" /><div className="font-medium text-brand">Upload foto produk</div><div className="text-xs text-stone-500">Feedify butuh foto produk asli untuk generate konten</div></div>
              )}
            </label>
          ...
          <div className="feedify-card p-6 space-y-4">
            <div>
              <h3 className="font-heading text-lg font-semibold text-brand mb-0.5">Konsep Visual</h3>
              <p className="text-[11px] text-stone-400">Tiap konsep punya ratusan sub-tema — hasil foto berbeda setiap generate meski konsep sama.</p>
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-2 xl:grid-cols-3 gap-2">
              {CONCEPT_OPTIONS.map(c => (
                <button key={c.id} type="button" data-testid={`concept-${c.id}`}
                  onClick={() => upd("composition_concept", form.composition_concept === c.id ? "" : c.id)}
                  className={`text-left p-3 rounded-xl border-2 transition-all btn-touch ${
                    form.composition_concept === c.id
                      ? "border-brand bg-brand-sand"
                      : "border-brand-sand hover:border-brand-light bg-white"
                  }`}>
                  <div className="text-base mb-1 leading-none">{c.icon}</div>
                  <div className="font-heading font-bold text-brand text-xs leading-tight">{c.name}</div>
                  <div className="text-[10px] text-stone-500 leading-tight mt-0.5">{c.desc}</div>
                </button>
              ))}
            </div>
            {!form.composition_concept && (
              <p className="text-[11px] text-stone-400 italic">Belum dipilih — sistem akan memilih konsep secara otomatis tiap generate.</p>
            )}
          </div>
          ...
          <div className="feedify-card p-6 space-y-4">
            <div>
              <h3 className="font-heading text-lg font-semibold text-brand mb-0.5">Tujuan Konten</h3>
              <p className="text-[11px] text-stone-400">Goal ini drastis mengubah arah visual, tipografi, dan pesan — pilih sesuai tujuan posting ini.</p>
            </div>
            <CampaignGoalSelector value={form.campaign_goal} onChange={v => upd("campaign_goal", v)} />
          </div>

          <div className="feedify-card p-6 space-y-4">
            <h3 className="font-heading text-lg font-semibold text-brand">Format</h3>
            <Field label="Aspect Ratio">
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                {ASPECT_RATIOS.map((r) => (
                  <button key={r.id} type="button" data-testid={`ratio-${r.id.split(" ")[0]}`}
                    onClick={() => upd("aspect_ratio", r.id)}
                    className={`p-3 rounded-xl border-2 text-xs font-semibold btn-touch ${form.aspect_ratio === r.id ? "border-brand bg-brand-sand text-brand" : "border-brand-sand text-stone-600"}`}>
                    <div className="flex justify-center mb-1">
                      <div className="border border-current rounded" style={{ width: r.w * 0.36, height: r.h * 0.36 }} />
                    </div>
                    {r.label}
                  </button>
                ))}
            ...
            {/* Style & placement override — advanced */}
            <button
              type="button"
              onClick={() => setShowAdvanced(v => !v)}
              className="flex items-center gap-2 text-xs text-stone-400 hover:text-brand transition-colors"
            >
              <SlidersHorizontal size={12} weight="duotone" />
              Override gaya visual & posisi
              {showAdvanced ? <CaretUp size={11} weight="bold" /> : <CaretDown size={11} weight="bold" />}
            </button>

            {showAdvanced && (
              <div className="space-y-4 pt-2 border-t border-brand-sand/60">
                <Field label="Style Visual">
                  <div className="flex flex-wrap gap-2">
                    {STYLE_PRESETS.map((s) => (
                      <button key={s} type="button" data-testid={`style-${s.toLowerCase().replace(/\s+/g, "-")}`}
                        onClick={() => upd("style_preset", s)}
                        className={`px-4 py-2.5 rounded-full text-sm font-medium border btn-touch ${form.style_preset === s ? "bg-brand text-brand-cream border-brand" : "bg-white border-brand-sand text-stone-700"}`}>
                        {s}
                      </button>
                    ))}
                  </div>
                ...
                <Field label="Posisi Produk">
                  <div className="flex flex-wrap gap-2">
                    {PLACEMENTS.map((p) => (
                      <button key={p.id} type="button" data-testid={`placement-${p.id}`}
                        onClick={() => upd("placement_rule", p.id)}
                        className={`px-4 py-2.5 rounded-full text-sm font-medium border btn-touch ${form.placement_rule === p.id ? "bg-brand-gold text-brand border-brand-gold" : "bg-white border-brand-sand text-stone-700"}`}>
                        {p.label}
                      </button>
                    ))}
                  </div>
          ...
          <div className={isAdmin ? "flex flex-col sm:flex-row gap-3" : ""}>
            ...
            <button
              ...
              className={`${isAdmin ? "flex-1" : "w-full"} py-4 bg-brand text-brand-cream rounded-full font-bold text-base hover:bg-brand-light btn-lift inline-flex items-center justify-center gap-2 disabled:opacity-50 shadow-lg`}
            >
              {generating ? <><CircleNotch size={20} className="animate-spin" /> Memproses (30-60 dtk)...</> : <><Sparkle size={20} weight="fill" /> Generate <span className="opacity-70 text-sm font-medium">(1 kredit)</span></>}
            </button>
          </div>

          {/* Prompt preview panel — admin only */}
        ...
        <div className="lg:sticky lg:top-6 lg:self-start space-y-4">
          ...
          <div className="feedify-card p-4 space-y-2.5">
            ...
            <BriefRow label="Style"    value={form.style_preset || "Minimal Clean"} />
            <BriefRow label="Konsep"   value={CONCEPT_OPTIONS.find(c => c.id === form.composition_concept)?.name || "Auto"} />
            <BriefRow label="Ratio"    value={form.aspect_ratio.split(" ")[0]} />
            <BriefRow label="Posisi"   value={PLACEMENTS.find(p => p.id === form.placement_rule)?.label || "Tengah"} />
            {form.headline && <BriefRow label="Headline" value={form.headline} truncate />}
          </div>
...
function BriefRow({ label, value, truncate }) {
  return (
    <div className="flex items-center justify-between text-xs gap-2">
      ...
      <span className={`font-semibold text-stone-700 text-right ${truncate ? "truncate max-w-[65%]" : "max-w-[65%] text-right"}`}>{value}</span>
    </div>
  );
}
